...
export const TOOL_REGISTRY: Tool[] = [
  ...
  // PDF Document Tools
  {
    ...
    inputSchema: {
      ...
      properties: {
        sources: {
          type: "array",
          items: { type: "string", description: "Supabase Storage path to PDF" },
          description: "Array of PDF storage paths to merge"
        },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        source: { type: "string", description: "Supabase Storage path to source PDF" },
        ranges: {
          type: "array",
          items: {
            type: "object",
            properties: {
              ...
              name: { type: "string", description: "Output filename for this range" }
            },
            required: ["start", "end"]
          },
          description: "Page ranges to extract"
        }
      },
  ...
  {
    name: "xmrt_pdf_sign",
    description: "Add a digital signature appearance to a PDF document",
    inputSchema: {
      type: "object",
      properties: {
        source: { type: "string", description: "Supabase Storage path to source PDF" },
        text: { type: "string", description: "Signature text", default: "Digitally Signed" },
        position: {
          type: "object",
          properties: {
            ...
            width: { type: "number", description: "Box width", default: 200 },
            height: { type: "number", description: "Box height", default: 60 }
          },
          description: "Signature box position"
        },
        reason: { type: "string", description: "Signature reason", default: "XMRT DAO Digital Signature" }
      },
      required: ["source"]
    }
  },
  {
    name: "xmrt_pdf_watermark",
    description: "Add a text watermark to all pages of a PDF",
    inputSchema: {
      type: "object",
      properties: {
        source: { type: "string", description: "Supabase Storage path to source PDF" },
        text: { type: "string", description: "Watermark text", default: "XMRT DAO Confidential" },
  ...
  {
    name: "xmrt_pdf_metadata",
    description: "Read or edit PDF document metadata (title, author, subject, keywords)",
    inputSchema: {
      type: "object",
      properties: {
        source: { type: "string", description: "Supabase Storage path to source PDF" },
        metadata: {
          ...
          description: "Metadata to update (omit fields to keep existing)"
        }
      },
      required: ["source"]
    }
  ...
  {
    name: "xmrt_pdf_compress",
    description: "Optimize and compress a PDF document to reduce file size",
    inputSchema: {
      type: "object",
      properties: {
        source: { type: "string", description: "Supabase Storage path to source PDF" },
  ...
  // Task & Agent Orchestration Tools (DenoClaw)
  {
    name: "xmrt_create_task",
    description: "Create a new agent task in the DenoClaw orchestration system",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: { type: "string", description: "Agent or user identifier" },
        objective: { type: "string", description: "Task objective description" },
  ...
  {
    name: "xmrt_decompose_task",
    description: "Decompose a task into sequenced operations for execution",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "xmrt_execute_task",
    description: "Execute the next pending operation for a task (respects 60s timeout)",
    inputSchema: {
      type: "object",
      properties: {
        task_id: { type: "string", description: "Task UUID" }
  ...
  {
    name: "xmrt_get_task_status",
    description: "Get full task status including all operations and results",
    inputSchema: {
      type: "object",
      properties: {
        task_id: { type: "string", description: "Task UUID" }
  ...
  {
    name: "xmrt_continue_checkpointed",
    description: "Resume execution of all checkpointed tasks (cron-style continuation)",
    inputSchema: {
      type: "object",
      properties: {}
    }
