...
export const XMRT_KNOWLEDGE_BASE: XMRTKnowledgeEntry[] = [
...

  {
    ...
    content: "The XMRT DAO frontend is served by a local Vite dev server (http://127.0.0.1:5173) backed by a local PostgREST/Supabase-replacement stack on port 54321 (local-sb). The frontend connects to the local-sb instance for all edge function and data operations, and communicates with Eliza via the local relay (:8080). Eliza can monitor frontend health, send webhooks for notifications, and coordinate backend-frontend integration. The architecture separates concerns: local-sb handles backend operations (database, AI, agents, mining) while the Vite dev server handles frontend presentation (React UI, user interactions).",
    category: 'deployment',
